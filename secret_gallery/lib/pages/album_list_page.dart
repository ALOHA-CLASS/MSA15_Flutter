...
class _AlbumListPageState extends State<AlbumListPage> {
  ...
  final _reorderScrollController = ScrollController();  // 드래그 정렬 스크롤 컨트롤러
  bool _customSortSelectMode = false; // 직접 정렬 모드에서 선택 삭제 활성화 여부
  bool get _showSelectAppBar => _selectedIds.isNotEmpty || _customSortSelectMode; // AppBar 표시조건
  ...
  Widget _buildNormalGrid() {
    ...
    return Listener(
      ...
      child: GridView.builder(
        // physics : 스클롤 물리 효과를 설정하는 속성
        // NeverScrollableScrollPhysics : 스크롤 비활성화
        // BouncingScrollPhysics        : 바운스 스크롤 효과
        ...
        gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(
          ...
          crossAxisSpacing: 12,   // 열 사이 간격
          mainAxisSpacing: 12,    // 행 사이 간격
          childAspectRatio: 0.9,  // 아이템의 가로 세로 비율 : 0.9
        ),
  ...
  Widget build(BuildContext context) {
    ...
    return PopScope(
      // *canPop : pop 가능 여부
      // 지금 화면이 뒤로 갈수 있는지 여부 지정
      // 선택모드도 아니고 검색모드도 아닌경우 pop 가능
