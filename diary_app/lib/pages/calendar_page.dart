...
class _CalendarPageState extends State<CalendarPage> {
  ...
  DateTime _focusedDay = DateTime.now();    // 현재 달력에서 지정한 날짜
  ...
  Future<void> _loadEntries() async {
    ...
    for (final e in entries) {
      ...
      // absent   : 없다, 부재
      // putIfAbsent(key, 함수)
      // : 첫번째 인자로 지정한 key 가 없으면,
      //   두번째 인자의 함수를 실행 후, 결과를 반환
      // ✨ 여기서는 해당 날짜 key 없으면, 
      //    새로운 [] 리스트를 생성하고 반환
      //    반환된 [] 리스트에 add() 메소드로 날짜 객체를 추가
  ...
  Future<void> _openDetail(DiaryEntry entry) async {
    ...
    // 상세화면으로 갔다가, 뒤로가기하면 다시 목록 갱신
    _loadEntries();
  }

  // 같은 날짜에 일기가 2개이상 있을 때
  // BottomSheet 를 띄워서 목록에서 선택하도록 하는 함수
