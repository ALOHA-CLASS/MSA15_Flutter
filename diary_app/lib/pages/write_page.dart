...
class _WritePageState extends State<WritePage> {
  ...
  String get _seletedDateStr => _seletedDate.toString().substring(0, 10);
  ...
  Future<void> _pickDate() async {
    final result = await showCalendarDatePicker2Dialog(
      ...
      config: CalendarDatePicker2WithActionButtonsConfig(
        ...
        okButton: 
          const Text('확인', style: TextStyle(color: Colors.amber),),
        cancelButton: 
          const Text('취소'),
      ), 
    ...
    if( result != null && result.isNotEmpty && result.first != null ) {
      setState(() => _seletedDate = result.first!);
    }
  }
  ...
  Future<void> _save() async {
    ...
    await _fileService.saveDiaryForDate(_seletedDateStr, title, content);
